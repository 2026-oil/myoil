...
CASE_SPECS = [
    ...
    CaseSpec(
        "aa_vanillatransformer",
        REPO_ROOT / "yaml/experiment/feature_set_aaforecast/aaforecast-vanillatransformer.yaml",
        1,
    ),
]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run and evaluate the Brent 4-way AAForecast bundle.")
    ...
    batch2 = [CASE_SPECS[2], CASE_SPECS[3]]
    ...
    cases = {
        ...
        "aa_vanillatransformer": completed["aa_vanillatransformer"]["models"]["AAForecast"],
    }
    ...
    transformer_keys = ["aa_informer", "aa_vanillatransformer"]
    transformer_cases = {name: cases[name] for name in transformer_keys}
    best_transformer_name, best_transformer = max(
        transformer_cases.items(),
        key=lambda item: item[1]["rank_score"],
    )
    best_non_aa_name, best_non_aa = max(
        {"plain_gru": cases["plain_gru"], "plain_informer": cases["plain_informer"]}.items(),
        key=lambda item: item[1]["rank_score"],
    )
    informer_primary_success = cases["aa_informer"]["strict_pass"] or cases["aa_informer"]["fallback_pass"]
    aa_gru_beats_non_aa = cases["aa_gru"]["rank_score"] > best_non_aa["rank_score"]
    order_ok = cases["aa_informer"]["rank_score"] > cases["aa_gru"]["rank_score"] > best_non_aa["rank_score"]
    plain_transformer_gap = best_transformer["rank_score"] - cases["plain_informer"]["rank_score"]
    plain_gru_gap = cases["aa_informer"]["rank_score"] - cases["plain_gru"]["rank_score"]
    ...
    metric = 1000.0 if success else (
        ...
        + max(0.0, cases["aa_informer"]["rank_score"] - best_non_aa["rank_score"])
        + 0.5 * max(0.0, cases["aa_gru"]["rank_score"] - best_non_aa["rank_score"])
    ...
    result = {
        ...
        "transformer_success": best_transformer["strict_pass"] or best_transformer["fallback_pass"],
        "informer_primary_success": informer_primary_success,
        "aa_gru_beats_non_aa": aa_gru_beats_non_aa,
        "best_non_aa": best_non_aa_name,
        "order_ok": order_ok,
        "best_transformer": best_transformer_name,
        "plain_transformer_gap": plain_transformer_gap,
        "plain_gru_gap": plain_gru_gap,
