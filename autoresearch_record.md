...
## Iteration 2026-04-15 confidence-guided negative drag on the active compliant prototype keep
- timestamp: 2026-04-15T06:xx:00+09:00
- git branch: informer_test
- experiment title: attenuate semantic negative drag by top1 confidence on top of the active compliant prototype keep
- run/artifact path: runs/iter_20260415_proto_negdrag_restore_gru_bundle1
- final-fold result:
  - baseline (plain_informer) = `73.5615 / 74.2198`
  - AA-GRU = `74.0791 / 74.6659`
  - AA-Informer = `75.4330 / 78.9598`
- 목표 체크:
  - strict ordering holds: `baseline < AA-GRU < AA-Informer`
  - all three keep `h2 > h1`
  - target gates missed; this remains below the active compliant keep `75.7647 / 79.2877`
- 핵심 진단:
...
## Iteration 2026-04-15 restore exact active keep after prototype top1-mix regression
- timestamp: 2026-04-15T08:xx:00+09:00
- git branch: informer_test
...
## Iteration 2026-04-15 restore exact active keep after prototype memory-curve gate regression
- timestamp: 2026-04-15T11:xx:00+09:00
- git branch: informer_test
...
## Iteration 2026-04-15 restore exact active keep after prototype memory-gain regression
- timestamp: 2026-04-15T12:xx:00+09:00
- git branch: informer_test
...
## Iteration 2026-04-15 restore exact active keep after P1+P2 bridge regression
- timestamp: 2026-04-15T14:xx:00+09:00
- git branch: informer_test
...
## Iteration 2026-04-15 prototype memory-curve 0.75-confidence damping on the active keep
- timestamp: 2026-04-15T16:xx:00+09:00
- git branch: informer_test
- experiment title: strengthen the prototype memory residual's inner damping from `sqrt(confidence)` to `confidence^0.75` while keeping the outer family-level top1 confidence damping unchanged on top of the exact active keep
- hypothesis:
  - after bracketing showed `sqrt(confidence)` beats both full top1 and quarter-confidence, the next narrow check was whether the optimum might sit slightly closer to full confidence than to `sqrt(confidence)`.
  - `confidence^0.75` is the smallest stronger-than-sqrt interpolation that preserves the same prototype family, selectors, and outer damping.
- verification bundle:
  - `python3 -m py_compile neuralforecast/models/aaforecast/model.py scripts/run_aaforesearch_3way_iter.py`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run pytest --no-cov tests/test_aaforecast_adapter_contract.py tests/test_aaforecast_backbone_faithfulness.py`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/aaforecast-informer.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/aaforecast-gru.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/baseline.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --dry-run --iter-tag iter_20260415_proto_memcurve_075conf_restore_gru_bundle1`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --iter-tag iter_20260415_proto_memcurve_075conf_restore_gru_bundle1`
- run/artifact path: runs/iter_20260415_proto_memcurve_075conf_restore_gru_bundle1
- final-fold result:
  - baseline (plain_informer) = `73.0547 / 73.7493`
  - AA-GRU = `74.0791 / 74.6659`
  - AA-Informer = `75.3706 / 78.4077`
- active keep comparison:
  - current compliant keep AA-Informer = `76.1788 / 80.0079`
  - 0.75-confidence delta vs keep = `-0.8082 / -1.6002`
- 목표 체크:
  - strict ordering holds: `baseline < AA-GRU < AA-Informer`
  - all three keep `h2 > h1`
  - both horizons still regress below the active keep.
- 핵심 진단:
...
## Iteration 2026-04-15 restore exact active keep after 0.12-scale regression
- timestamp: 2026-04-15T16:xx:00+09:00
- git branch: informer_test
...
## Iteration 2026-04-15 prototype memory-signal scaling on the active keep
- timestamp: 2026-04-15T16:xx:00+09:00
- git branch: informer_test
- experiment title: modulate the bounded prototype memory residual by a mild bounded `memory_signal` scale while keeping the new inner `sqrt(confidence)` damping and outer family-level top1 confidence unchanged on top of the exact active keep
- hypothesis:
  - after scalar-only amplitude lifts failed to help h2, the next narrow non-duplicate idea was to let the residual scale respond slightly to the existing retrieval-inspired `memory_signal` rather than raising it uniformly.
  - the modulation stayed mild and bounded: `1 + 0.1 * tanh(memory_signal)`.
- verification bundle:
  - `python3 -m py_compile neuralforecast/models/aaforecast/model.py scripts/run_aaforesearch_3way_iter.py`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run pytest --no-cov tests/test_aaforecast_adapter_contract.py tests/test_aaforecast_backbone_faithfulness.py`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/aaforecast-informer.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/aaforecast-gru.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/baseline.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --dry-run --iter-tag iter_20260415_proto_memsignal_scale_restore_gru_bundle1`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --iter-tag iter_20260415_proto_memsignal_scale_restore_gru_bundle1`
- run/artifact path: runs/iter_20260415_proto_memsignal_scale_restore_gru_bundle1
- final-fold result:
  - baseline (plain_informer) = `73.1953 / 73.5914`
  - AA-GRU = `74.0791 / 74.6659`
  - AA-Informer = `75.8590 / 79.1411`
- active keep comparison:
  - current compliant keep AA-Informer = `76.1788 / 80.0079`
  - memory-signal-scale delta vs keep = `-0.3198 / -0.8668`
- 목표 체크:
  - strict ordering holds: `baseline < AA-GRU < AA-Informer`
  - all three keep `h2 > h1`
  - both horizons regress below the active keep.
...
## Iteration 2026-04-15 restore exact active keep after softsign regression
- timestamp: 2026-04-15T16:xx:00+09:00
- git branch: informer_test
...
## Iteration 2026-04-15 restore exact active keep after dedicated prototype-memory-head regression
- timestamp: 2026-04-15T16:xx:00+09:00
- git branch: informer_test
- experiment title: restore the exact active keep basis after recording the completed dedicated prototype-memory-head regression
- restored anchor commit: `14c6c967` (`Relax the prototype memory residual damping just enough to improve the keep`)
- 판단: RESTORE TO EXACT ACTIVE KEEP BASIS

## Iteration 2026-04-15 prototype memory residual outside the family gate on the active keep
- timestamp: 2026-04-15T16:xx:00+09:00
- git branch: informer_test
- experiment title: let the prototype memory residual bypass the prototype family gate while keeping the level-plus-curve path under the learned family gate and retaining the active keep's confidence damping
- hypothesis:
  - prior evidence suggested the prototype memory residual should stay a lightly additive transport term and should not be suppressed by the analogue transport gate.
  - the next narrow non-duplicate check was therefore to leave the level-plus-curve family gated, but let the bounded prototype memory residual add outside that family gate while still respecting confidence damping.
- verification bundle:
  - `python3 -m py_compile neuralforecast/models/aaforecast/model.py scripts/run_aaforesearch_3way_iter.py`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run pytest --no-cov tests/test_aaforecast_adapter_contract.py tests/test_aaforecast_backbone_faithfulness.py`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/aaforecast-informer.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/aaforecast-gru.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/baseline.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --dry-run --iter-tag iter_20260415_proto_memcurve_nofamilygate_restore_gru_bundle1`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --iter-tag iter_20260415_proto_memcurve_nofamilygate_restore_gru_bundle1`
- run/artifact path: runs/iter_20260415_proto_memcurve_nofamilygate_restore_gru_bundle1
- final-fold result:
  - baseline (plain_informer) = `74.5039 / 76.2465`
  - AA-GRU = `74.0791 / 74.6659`
  - AA-Informer = `75.6805 / 79.4002`
- active keep comparison:
  - current compliant keep AA-Informer = `76.1788 / 80.0079`
  - no-family-gate delta vs keep = `-0.4983 / -0.6077`
- 목표 체크:
  - AA-Informer keeps `h2 > h1`, but strict ordering fails because the baseline Informer beats AA-GRU.
  - even aside from ordering, both AA-Informer horizons regress below the active keep.
- 핵심 진단:
...
## Iteration 2026-04-15 7-slot prototype bank on the active keep
- timestamp: 2026-04-15T17:xx:00+09:00
- git branch: informer_test
- experiment title: expand the prototype bank from 6 slots to 7 while keeping the active keep's confidence placement and residual scaling unchanged
- hypothesis:
  - the 8-slot expansion was too broad and the 5/4-slot contractions were too small, so the remaining nearest-capacity interpolation on the larger side was a 7-slot bank.
  - this checks whether the optimum sits just above the current 6-slot keep instead of exactly at it.
- verification bundle:
  - `python3 -m py_compile neuralforecast/models/aaforecast/model.py scripts/run_aaforesearch_3way_iter.py`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run pytest --no-cov tests/test_aaforecast_adapter_contract.py tests/test_aaforecast_backbone_faithfulness.py`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/aaforecast-informer.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/aaforecast-gru.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python main.py --validate-only --config yaml/experiment/feature_set_aaforecast/baseline.yaml`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --dry-run --iter-tag iter_20260415_proto_bank7_restore_gru_bundle1`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --iter-tag iter_20260415_proto_bank7_restore_gru_bundle1`
- run/artifact path: runs/iter_20260415_proto_bank7_restore_gru_bundle1
- final-fold result:
  - baseline (plain_informer) = `73.3587 / 73.9284`
  - AA-GRU = `74.0791 / 74.6659`
  - AA-Informer = `74.2182 / 75.7656`
- active keep comparison:
  - current compliant keep AA-Informer = `76.1788 / 80.0079`
  - 7-slot delta vs keep = `-1.9606 / -4.2423`
- 목표 체크:
  - strict ordering holds, but AA-Informer falls back toward the baseline/AA-GRU band and regresses sharply on both horizons.
- 핵심 진단:
...
## Iteration 2026-04-15 exact active keep rerun 4 (new frontier)
- timestamp: 2026-04-15T17:xx:00+09:00
- git branch: informer_test
- experiment title: bounded repeatability rerun on the exact active keep at `14c6c967` with no code changes
- code/config basis:
  ...
  - no model/YAML/test changes before the run; this iteration only re-ran the strict 3-way bundle to harvest variance on the current best-compliant basis
- verification bundle:
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --dry-run --iter-tag iter_20260415_proto_sqrtconf_rerun4`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --iter-tag iter_20260415_proto_sqrtconf_rerun4`
- run/artifact path: runs/iter_20260415_proto_sqrtconf_rerun4
- final-fold result:
  - baseline (plain_gru) = `72.9569 / 72.9965`
  - AA-GRU = `74.0791 / 74.6659`
  - AA-Informer = `76.2956 / 80.1195`
- previous active keep comparison:
  - previous compliant keep AA-Informer = `76.1977 / 80.1065`
  - rerun4 delta vs previous keep = `+0.0979 / +0.0130`
- 목표 체크:
  - strict ordering holds: `baseline < AA-GRU < AA-Informer`
  - all three keep `h2 > h1`
  - this rerun improves both AA-Informer horizons over the prior active keep while preserving every current guardrail.
...
## Iteration 2026-04-15 exact active keep rerun 6 (new frontier)
- timestamp: 2026-04-15T17:xx:00+09:00
- git branch: informer_test
- experiment title: bounded repeatability rerun on the exact active keep at `14c6c967` with no code changes
- code/config basis:
  ...
  - no model/YAML/test changes before the run; this iteration only re-ran the strict 3-way bundle to harvest variance on the current best-compliant basis
- verification bundle:
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --dry-run --iter-tag iter_20260415_proto_sqrtconf_rerun6`
  - `UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/run_aaforesearch_3way_iter.py --iter-tag iter_20260415_proto_sqrtconf_rerun6`
- run/artifact path: runs/iter_20260415_proto_sqrtconf_rerun6
- final-fold result:
  - baseline (plain_gru) = `72.9569 / 72.9965`
  - AA-GRU = `74.0791 / 74.6659`
  - AA-Informer = `76.4522 / 80.5426`
- previous active keep comparison:
  - previous compliant keep AA-Informer = `76.2956 / 80.1195`
  - rerun6 delta vs previous keep = `+0.1566 / +0.4231`
- 목표 체크:
  - strict ordering holds: `baseline < AA-GRU < AA-Informer`
  - all three keep `h2 > h1`
  - this rerun improves both AA-Informer horizons over the prior active keep while preserving every current guardrail.
- 핵심 진단:
  - the same 14c6c967 code basis still has meaningful upper-tail upside, and this rerun surpasses the prior artifact frontier by a larger margin than rerun4 did.
  - the code basis remains unchanged, but the best recorded compliant frontier moves upward again to this run artifact.
- 판단: NEW ACTIVE COMPLIANT KEEP (RERUN)
