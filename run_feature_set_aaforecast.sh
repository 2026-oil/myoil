...
configs=(
  "yaml/experiment/feature_set_aaforecast_dubai/baseline.yaml"
...
export NF_CASE_LOG_ROOT="${NF_CASE_LOG_ROOT:-runs/_batch_logs/feature_set_aaforecast_dubai_matrix}"
export NF_CASE_TIMESTAMP="${NF_CASE_TIMESTAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
export NF_FEATURE_SET_RAW_ROOT="${NF_FEATURE_SET_RAW_ROOT:-runs/raw_feature_set_aaforecast_dubai}"
...
echo "[feature_set_aaforecast_dubai_matrix] configs=${#configs[@]}"
echo "[feature_set_aaforecast_dubai_matrix] batch_id=$batch_id"
echo "[feature_set_aaforecast_dubai_matrix] raw_batch_root=$raw_batch_root"
...
if [[ -f "$summary_json" ]]; then
  "$post_python" "$repo_root/scripts/feature_set_aaforecast_dubai_postprocess.py" \
    --summary-json "$summary_json" \
    --raw-batch-root "$raw_batch_root" \
    --x-start "$NF_FEATURE_SET_GRAPH_X_START" \
    --x-end "$NF_FEATURE_SET_GRAPH_X_END" || post_status=$?
else
  echo "[feature_set_aaforecast_dubai_matrix] missing summary_json=$summary_json" >&2
  post_status=1
fi
