...
echo "[feature_set_aaforecast_matrix] batch_id=$batch_id"
echo "[feature_set_aaforecast_matrix] raw_batch_root=$raw_batch_root"
...
if [[ -f "$summary_json" ]]; then
  ...
  echo "[feature_set_aaforecast_matrix] missing summary_json=$summary_json" >&2
  post_status=1
fi
