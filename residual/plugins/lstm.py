...
class LSTMResidualPlugin(ResidualPlugin):
    ...
    def __init__(self, *, lookback: int = 4, hidden_size: int = 8, num_layers: int = 1, epochs: int = 20, learning_rate: float = 0.01):
    ...
    def _tensorize(self, values: list[float]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if not xs:
            raise ValueError('Residual train series is too short for configured lookback')
