...
- high-VIF counts: raw=87, diff1=23
...
- predictors used: 89
- high-VIF predictors: 87 | severe predictors: 82
- condition number: 2251814753260314.50 (warning)
- min eigenvalue: 3.93465e-27; eigenvalue ratio: 1.97213e-31; near singularity: yes
...
- group_7: keep `Idx_Shanghai50` as the first representative candidate; members [Idx_CH50, Idx_CSI300, Idx_Shanghai50] form a redundant block (max VIF=699.70, max |corr|=0.962).
- group_3: keep `Bonds_BRZ_10Y` as the first representative candidate; members [Bonds_BRZ_10Y, Bonds_BRZ_1Y] form a redundant block (max VIF=208.11, max |corr|=0.934).
- group_4: keep `Bonds_IND_10Y` as the first representative candidate; members [Bonds_IND_10Y, Bonds_IND_1Y] form a redundant block (max VIF=176.22, max |corr|=0.861).
- group_6: keep `Com_LME_Zn_Inv` as the first representative candidate; members [Com_LME_Al_Inv, Com_LME_Ni_Inv, Com_LME_Zn_Inv] form a redundant block (max VIF=108.27, max |corr|=0.863).
- group_5: keep `Com_Cheese` as the first representative candidate; members [Com_Cheese, Com_Milk] form a redundant block (max VIF=43.79, max |corr|=0.938).

### diff1
- observations used: 583
- predictors used: 89
- high-VIF predictors: 23 | severe predictors: 19
- condition number: 2787760986321743.50 (warning)
- min eigenvalue: 8.32969e-28; eigenvalue ratio: 1.28673e-31; near singularity: yes
...
- group_2: keep `Bonds_MOVE` as the first representative candidate; members [BS_Core_Index_B, Bonds_MOVE] form a redundant block (max VIF=4503599627370496.00, max |corr|=0.862).
- group_8: keep `Idx_SnP500` as the first representative candidate; members [Idx_SnP500, Idx_SnPGlobal1200] form a redundant block (max VIF=43.28, max |corr|=0.963).
- group_7: keep `Idx_Shanghai` as the first representative candidate; members [Idx_CH50, Idx_CSI300, Idx_Shanghai, Idx_Shanghai50] form a redundant block (max VIF=32.07, max |corr|=0.957).
- group_6: keep `Com_LME_Cu_Cash` as the first representative candidate; members [Com_LMEX, Com_LME_Cu_Cash] form a redundant block (max VIF=27.02, max |corr|=0.903).
...
- group_5: keep `Com_Gasoline` as the first representative candidate; members [Com_BrentCrudeOil, Com_Gasoline] form a redundant block (max VIF=9.31, max |corr|=0.817).
...
### Raw vs diff comparison
- high-VIF counts: raw=87, diff1=23
- severe-VIF counts: raw=82, diff1=20
...
- predictors used: 89
- high-VIF predictors: 87 | severe predictors: 82
- condition number: 2251814771589096.00 (warning)
- min eigenvalue: 3.93926e-27; eigenvalue ratio: 1.97213e-31; near singularity: yes
...
- observations used: 583
- predictors used: 89
- high-VIF predictors: 23 | severe predictors: 20
- condition number: 2787773033514679.00 (warning)
- min eigenvalue: 8.32283e-28; eigenvalue ratio: 1.28672e-31; near singularity: yes
...
- group_7: keep `Idx_SnP500` as the first representative candidate; members [Idx_SnP500, Idx_SnPGlobal1200] form a redundant block (max VIF=43.28, max |corr|=0.963).
- group_6: keep `Idx_Shanghai` as the first representative candidate; members [Idx_CH50, Idx_CSI300, Idx_Shanghai, Idx_Shanghai50] form a redundant block (max VIF=32.07, max |corr|=0.957).
- group_5: keep `Com_LME_Cu_Cash` as the first representative candidate; members [Com_LMEX, Com_LME_Cu_Cash] form a redundant block (max VIF=27.02, max |corr|=0.903).
- group_4: keep `Bonds_US_1Y` as the first representative candidate; members [Bonds_US_1Y, Bonds_US_2Y] form a redundant block (max VIF=10.83, max |corr|=0.864).
- group_3: keep `Bonds_AUS_10Y` as the first representative candidate; members [Bonds_AUS_10Y, Bonds_US_10Y] form a redundant block (max VIF=9.18, max |corr|=0.826).
