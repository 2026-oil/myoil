...
from neuralforecast.auto import AutoCMamba
...
def test_cmamba_n_series_1(suppress_warnings):
    _run_smoke(n_series=1)


def test_cmamba_n_series_5(suppress_warnings):
    _run_smoke(n_series=5)


def test_autocmamba(setup_dataset):
    check_args(AutoCMamba, exclude_args=["cls_model"])

    my_config = AutoCMamba.get_default_config(h=12, n_series=1, backend="optuna")
    ...
    my_config = AutoCMamba.get_default_config(h=12, n_series=1, backend="ray")
