...
SCRIPT_PATH = REPO_ROOT / "run_case3_bomb.sh"
...
def test_run_case3_bomb_script_writes_logs_and_summary(tmp_path: Path) -> None:
...
def test_run_case3_bomb_script_default_config_list_has_declared_order() -> None:
    ...
    for config_path in expected:
        needle = f"{config_path}\n"
        idx = script.find(needle)
        assert idx >= 0, f"missing {config_path}"
